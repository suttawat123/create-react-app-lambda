...
import { fetchCreatePos } from "../../services/Pos/action";
...
class PosTaxInvoice extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoadData: false,
  ...
  printBill = async (status) => {
    ...
    if (!this.state.isValidateCustomer && !this.state.isValidateDetail) {
      ...
      body.status = status;
      body.real_sale = false;

      const { fetchCreatePos } = this.props;
      let response = await fetchCreatePos(body);
        ...
        {this.state.dataPos.detail.map((m, index) => {
          return (
            ...
            >
              ...
              {/* <td className="textCenter">{index}</td> */}
              <td className="">
                <Input
                  className="input"
                  type="text"
    ...
    const CustomInputStartDate = React.forwardRef((props, ref) => {
      return (
        <div>
          <Input
            className={
              this.state.isValidateStartDate
                ? "txt-search-input no-margin is-invalid"
                : "txt-search-input no-margin"
            }
            value={props.value}
                ...
                <Col sm={12} md={4} lg={4}>
                  <FormGroup>
                    ...
                    <DatePicker
                      id="dateStart"
                      name="dateStart"
                      selected={new Date(dataPos.sale_date)}
                      onChange={(date) => {
  ...
  bindActionCreators(
    {
      ...
      fetchCreatePos: fetchCreatePos,
      fetchDataCategoryProductNoLimit: fetchDataCategoryProductNoLimit,
    },
    dispatch
  );

export default connect(mapStateToProps, mapDispatchToProps)(PosTaxInvoice);
